...
similarities = {}
frontEndVector = []
scrapedVector = list(testInput.values())
for word in list(testInput.keys()): #need only dict keys, need to convert the returned value froms .keys() from a dict object to a list
	try:
...
print(np.linalg.norm(scrapedVector))
print(np.linalg.norm(frontEndVector))
